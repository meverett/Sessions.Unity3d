...
namespace UnityEngine.EventSystems
{
    ...
    /// </summary>
    public class SessionsOVRInputModule : PointerInputModule
    {
        ...
        public float maxPointerLength = 50;
        ...
        protected SessionsOVRInputModule()
        { }
        ...
        virtual protected MouseState GetGazePointerData()
        {
            ...
            // If a line renderer is present, max sure it's rendered length does not exceed the define max length
            if (maxPointerLength > 0 && lineRenderer != null)
            {
                var start = lineRenderer.GetPosition(0);
                var end = lineRenderer.GetPosition(1);

                if (Vector3.Distance(start, end) > maxPointerLength)
                {
                    var delta = end - start;
                    delta.Normalize();
                    delta *= maxPointerLength;
